((window) => {
	...
	const autoInstrumentButtons = () => {
		...
		document.addEventListener("click", (event) => {
			...
			if (button) {
				// Skip <a> tags handled by link tracking (auto or data-driven)
				if (button.tagName === 'A' && (window.Fusionaly.config.autoInstrumentLinks || hasDataAttribute(button, 'event-name'))) {
					return;
				}
	...
	const autoInstrumentLinks = () => {
		...
		document.addEventListener("click", (event) => {
			...
			if (eventName) {
				// For external navigation links, use sendBeacon to survive page unload
